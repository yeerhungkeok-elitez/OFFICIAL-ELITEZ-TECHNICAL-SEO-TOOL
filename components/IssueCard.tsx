...
function RiskBadge({ risk }: { risk: string }) {
  const map = {
    'safe':               { cls: 'bg-green-50 text-green-700 border border-green-200',  icon: '✅', label: 'Safe to Fix' },
  ...
  const { cls, icon, label } = map[risk as keyof typeof map] ?? { cls: 'badge-neutral', icon: '', label: risk };
  return (
...
export default function IssueCard({ issue, defaultExpanded = false }: Props) {
  const [expanded, setExpanded]     = useState(defaultExpanded);
  const [viewMode, setViewMode]     = useState<'client' | 'developer'>('client');
  const [showAllPages, setShowAll]  = useState(false);
  ...
  const bgHover = {
    critical: 'hover:bg-red-50/40',
    high:     'hover:bg-orange-50/40',
    medium:   'hover:bg-yellow-50/30',
    low:      'hover:bg-blue-50/30',
  }[issue.severity] ?? 'hover:bg-slate-50';

  const visiblePages = showAllPages ? issue.affectedPages : issue.affectedPages.slice(0, 5);

  return (
    <div className={`card border-l-4 ${borderColor} transition-all`}>
      {/* Header — always visible, click to expand */}
      <button
        onClick={() => setExpanded(e => !e)}
        className={`w-full text-left px-5 py-4 flex items-start gap-4 ${bgHover} transition-colors rounded-xl`}
      >
        ...
        <div className="flex items-center gap-3 flex-shrink-0">
          <div className="text-right">
            <p className="text-xs text-slate-400 font-medium">Affected</p>
            <p className="font-bold text-slate-700 text-sm">{issue.count} page{issue.count !== 1 ? 's' : ''}</p>
          </div>
          ...
          {viewMode === 'client' ? (
            ...
            <div className="space-y-4">
              <div className="bg-slate-50 rounded-xl p-4">
                ...
                <p className="text-sm text-slate-700 leading-relaxed font-mono">{issue.technicalDetail}</p>
              </div>
              <div>
                <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2">
              ...
              <div className="flex flex-wrap gap-4 text-xs text-slate-500">
                <span>Risk: <strong className="text-slate-700">{issue.riskLevel}</strong></span>
                <span>Category: <strong className="text-slate-700">{issue.category}</strong></span>
                <span>Auto-fixable: <strong className="text-slate-700">{issue.autoFixable ? 'Yes' : 'No'}</strong></span>
              </div>
            </div>
          )}
              ...
              <div className="space-y-1">
                ...
                {issue.affectedPages.length > 5 && (
                  ...
                  >
                    {showAllPages
                      ? '▴ Show fewer'
                      : `▾ Show all ${issue.affectedPages.length} URLs`}
                  </button>
                )}
              </div>
            </div>
