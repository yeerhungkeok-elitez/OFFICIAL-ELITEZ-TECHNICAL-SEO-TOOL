...
// ─────────────────────────────────────────────────────────────────────────────
// Elitez Technical SEO Doctor — Resolved Issues Table (V6)
// Issues present in the "before" snapshot but absent in the "after" snapshot.
// ─────────────────────────────────────────────────────────────────────────────
...
export default function ResolvedIssuesTable({ issues }: Props) {
  if (issues.length === 0) {
    return (
      <div className="text-center py-8 text-sm text-slate-400">
        No resolved issues detected in this comparison period.
      </div>
    );
  }
    ...
    <div className="overflow-x-auto rounded-xl border border-emerald-200">
      <table className="w-full text-sm">
        ...
        <tbody className="bg-white divide-y divide-emerald-100">
          {sorted.map(fp => (
            <tr key={fp.key} className="hover:bg-emerald-50/40 transition-colors">
